...
def config():
	...
	parser.add_argument('--tau', type=float, default=0.005, help="Soft update co-efficient")
	...
	argList = {'LunarLander-v2': ['render_mode', 'continuous', 'gravity', 'enable_wind', 'wind_power', 'turbulence_power'],
			   ...
			   'MountainCar-v0': ['render_mode'],
			   'Pendulum-v1': ['render_mode', 'g']}
	env_kwargs = {'render_mode': args.guiMode, 'continuous':args.continuous, 'gravity': args.gravity, 'g': -args.gravity, 'enable_wind': args.wind, 'wind_power': args.windPower, 'turbulence_power': args.turbulencePower}
	args.env_kwargs = {}
	if args.intuitiveEncouragement:
		try:
			...
			console_out(consoleMsg=f'Unsupported environment "{args.env}". Check arg "env" for typos, or add support', msgCategory='FATAL')
			sys.exit(-1)
	# sanity checks
	if len(args.learning_rate) != len(args.lr_schedule):
		console_out(consoleMsg='Args learning-rate and lr-schedule have different lengths!', msgCategory='FATAL')
		sys.exit(-1)
	if len(args.clip_range) != len(args.clip_schedule):
		console_out(consoleMsg='Args clip-range and clip-schedule have different lengths!', msgCategory='FATAL')
		sys.exit(-1)
	if len(args.intuitionCoef) != len(args.intuitionCoef_schedule):
		console_out(consoleMsg='Args intuitionCoef and intuitionCoef-schedule have different lengths!', msgCategory='FATAL')
		sys.exit(-1)
	if args.continuous and args.algorithm == 'dqn':
		console_out(consoleMsg='DQN does not support continuous action spaces. Remove the --continuous flag or change the optimisation algorithm!', msgCategory='FATAL')
		sys.exit(-1)
