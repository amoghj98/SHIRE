...
class intuitiveTRPO(OnPolicyAlgorithm):
	"""
	...
	def __init__(
		...
		if self.use_intuition:
			...
			self.env_kwargs = {'debug': False}
			self.net = inets.__envs__[self.env_name](**self.env_kwargs)
		#
	...
	def train(self) -> None:
		...
		for rollout_data in self.rollout_buffer.get(batch_size=None):
			...
			if self.use_intuition:
				...
				# surrogate policy objective
				policy_objective = (advantages * ratio).mean() + intuition_coef * intuitive_action_loss
			else:
				policy_objective = (advantages * ratio).mean()
